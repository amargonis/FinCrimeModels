...
def cluster_explainability(conn, anomaly_features_names, 
    ...
    models = {}
    columnsList =["datascience_model_version", "datascience_model_id", 
        "cluster_id","feature", "feature_importance", "scaled_centroid", 
    ...
    for anom_feature in anomaly_features_names:    
        ...
        try:
            ...
            print("""error creating feature value, 
                this can happen when a feature is used for clustering and 
                anomaly detection and can be ignored for duplicate row errors""")
            print(inst)
...
def train(data_conf, model_conf, **kwargs):
    """
        Main entry point to begin the Cluster operations.
    """
	#def train(context: ModelContext, **kwargs):
    print("=====BEGIN: train anomaly model")
    ...
    tdFC.initFC(data_conf["featureStore_db_name"], 
        data_conf["dataScience_db_name"], data_conf["metadata_db_name"] )
    print("step_1")

    # AOA model ID and Version GUID
    # If not using Teradata ModelOps, simply pass user defined values
    ...
    print("step_3")
    # For successful clustering, data should be rescaled to 0 - 1 
    ...
    if len(categoric_feature_names) > 0 and len(numeric_feature_names) > 0:
        ...
        cluster_ready_data = (
            categoric_feature_values if len(categoric_feature_names) > 0 
            else cluster_features_scaled
        )
    ...
    if hyperparams["cluster_mode"] == "auto":
        ...
        for i in range(2,int(hyperparams["max_clusters"])):
            km = KMeans(n_clusters=i,init=hyperparams["init"], 
                max_iter=hyperparams["max_iter"], n_init=hyperparams["n_init"]).fit(cluster_ready_data)
    ...
    clustered_model = KMeans(n_clusters=clusterCount,init=hyperparams["init"], 
        max_iter=int(hyperparams["max_iter"]), 
        n_init=int(hyperparams["n_init"])).fit(cluster_ready_data)
    
    centroids = pd.DataFrame(clustered_model.cluster_centers_, 
        columns=list(map(lambda x: str(x) + "_centroid", 
        cluster_features_names.split(","))))
    
    print("step_6")
    loadModel(data_conf, clustered_model, featureSetVersion, 
        featureSetId, model_version, model_ID, conn)
    #print("step_7")
    ...
    cluster_explainability(conn, anomaly_features_names, ID, pdfSourceData, 
        hyperparams["training_date"], clusterCount, centroids, avg_values, 
        min_values, max_values, std_dev, str(model_version), str(model_ID), 
