...
def create_ADS(data_conf, src_data, ID, label, features, dataset_version, dataset_ID, conn):
    query = f"""create multiset table {data_conf["ADS_db_name"]}.BO_ADS_Dv_{dataset_version}_D_{dataset_ID} as (
                select {ID},{data_conf["Date_col_name"]},{features},{label},LAG({label},{data_conf["prediction_interval"]}) over( partition by {ID} order by {data_conf["Date_col_name"]} DESC) as fraud
            from {data_conf["ADS_db_name"]}.{src_data}) with data primary index({ID})"""

    try:
        conn.execute(f"""Drop table {data_conf["ADS_db_name"]}.BO_ADS_Dv_{dataset_version}_D_{dataset_ID}""")
        conn.execute(query)
    except Exception as e:
        print(e)
        conn.execute(query)
...
def get_train_data(data_conf, ID, label, conn, dataset_version, dataset_ID):
    X = pd.read_sql(f"""select * from {data_conf["ADS_db_name"]}.BO_ADS_Dv_{dataset_version}_D_{dataset_ID}""", conn)
    ...
    y = X["fraud"]
    X = X.drop([data_conf["Date_col_name"], label, "fraud"], axis=1)
    for column in X.select_dtypes(include='object').columns:
        X = pd.concat([X.drop(column, axis=1), pd.get_dummies(X[column], prefix=column)], axis=1)
    X = X.astype(float)
    X = X.set_index(ID)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=data_conf['test_size'], random_state=42)
...
def delete_record_if_exists(db_name, model_version, conn):
    conn.execute(f"""delete from {db_name}.models_artifacts_BO WHERE model_version='{model_version}'""")


def if_not_exist_create_table(db_name, conn):
    ret = pd.read_sql(
        f"select distinct TableName FROM DBC.TablesV WHERE DatabaseName= '{db_name}' and TableName='models_artifacts_BO'",
        conn)

    if not len(ret) > 0:
        conn.execute(f"""CREATE MULTISET TABLE {db_name}.models_artifacts_BO, FALLBACK , NO BEFORE JOURNAL,
...
def loadModel(data_conf, classifier, dataset_version, dataset_ID, model_version, model_ID, conn):
    ...
    conn.execute(
        f"""insert into {data_conf["datascience_db"]}.models_artifacts_BO(model_version, model_id, model, skmodel, 
...
#             "ADS_name": "BO_ADS_MS_1",
#             "Date_col_name": "fc_agg_summary_date",
#             "prediction_interval": 1,
#             "metadata_db_name": "fincrime_bustout_dev",
#             "metadata_dataset_name": "model_feature2",
#             "dataset_version": "1",
#             "dataset_ID": "1",
#             "sampling": "True",
#             "sampling_rate": 0.5,
#             "Test_ADS_db_name": "fincrime_bustout_dev",
#             "Test_ADS_name": "BO_Test_ADS_1",
#             "test_size": 0.2,
#             "Score_ADS_db_name": "fincrime_bustout_dev",
#             "Score_ADS_name": "BO_Score_ADS_1",
...
#
#         train(data_conf, model_conf, model_version='123', model_id='123')
#
#     except Exception as e:
#         print(e)
